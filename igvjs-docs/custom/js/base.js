...
function buildAddItem(ul, pageUrl, liClass) {
  return function(tocItem) {
    var tocLink = $('<a class="wm-article-link wm-page-toc-text">')
    ...
    if (tocItem.children && tocItem.children.length > 0) {
      var ulChild = $('<ul class="wm-toctree-child">');
      tocLink.addClass('wm-toc-text')
      ulChild.hide();
      tocLink.click(function() {
        ulChild.slideToggle();$(this).toggleClass('wm-page-toc-open');
      })
      tocItem.children.forEach(buildAddItem(ulChild, pageUrl, "wm-toc-li-child"));
      li.append(ulChild);
    }
    ul.append(li);
  }
...
function renderPageToc(parentElem, pageUrl, pageToc) {
  var ulRoot = $('<ul class="wm-toctree">');
  pageToc.forEach(buildAddItem(ulRoot, pageUrl, "wm-toc-li"));
  ...
  $('<li class="wm-page-toc wm-toc-li-nested collapse">').append(ulRoot).insertAfter(parentElem)
    .collapse(showPageToc ? 'show' : 'hide');
}

function orig_renderPageToc(parentElem, pageUrl, pageToc) {
